...
class Normal:
  '''
  normal class with pdf function
  '''
  ...
  def x_value(self, z):
    '''
    x_value function returns the x value of z
    '''
    return self.mean + z * self.stddev

  def pdf(self, x):
    '''
    pdf function returns xth value of normal distribution
    '''
    return (1 / (self.stddev * (2 * 3.1415926536) ** 0.5)) * 2.7182818285 ** (-0.5 * ((x - self.mean) / self.stddev) ** 2)

  def cdf(self, x):
    '''
    cdf function returns the sum of the xth value of normal distribution
    '''
